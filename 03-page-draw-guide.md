...
> ⭐ **先讀 [01-seat-rules](01-seat-rules.md)**（要牢記的那一頁）:② 盡量發揮、③ 才收斂;
...
| **① 形狀** | 建頁（**建頁即公開** —— ② ③ 需要；不想被搜到就設 noindex，寫法看 `playbook-get name="guide/rank-math"` 的「noindex 怎麼寫」）、插 block、調 repeater 格數、藏多餘格、版面屬性、錨點；**站主已經給的文字**一趟 batch 填進去 | `page-batch`（add-section / fill-page-text / set-item-visibility / set-block-anchor）、`block-set-attribute` | **不寫新文案、不打磨字**；沒給文字的槽留示範文字（示範文字在這一段**不是問題**）；沒給圖的槽同樣留模板示範圖（形狀段不是問題，內容段再換）；不寫草稿層、不碰背景 | 前台網址 + 「哪幾顆請你看形狀」。站主逐顆說**留 / 換**；換的就移除重插，文字跟著搬 |
...
- **內容列表類 block（文章 / Podcast / 商品 / 影片）的資料來自後台，不填字**：① 段用 `block-set-listing` 設查詢條件（怎麼設看 `playbook-get name="guide/block/listing"`），後台沒內容畫面就是空的（照實回報，⛔ 不要拿別的 block 手填假清單）。要建示範內容（文章 / 商品）**先問站主**，工具在 `guide/content`。
...
- **狀態偽類**（`:hover` / `:focus` / `:focus-visible` / `:active`）要寫在**地址**上、放在選擇器**最後**才翻得動（例 `[data-hz-surface="…"]:hover`，③ 收回時搬進自訂樣式段）。仍翻不動的：偽元素（`::before` / `::after`）、掛在整頁字義 class 上（`.hz-typo-typo-NN:hover`）、放在中間（`.a:hover .b`）、`:not(:hover)`、`:visited` —— 改寫法或拿掉那條、回報一聲（說法以 `guide/design/locate` 的 `unsupported_selector` 那一列為準）。
...
- ⚠️ 結構偽類要寫在**地址**上（例 `[data-hz-text="…"]:first-child`）。⛔ 不要掛在字義 class 上（`.hz-typo-typo-03:not(.x)`）—— 字義 class 的意思是「這個字整站一起變」，掛了偽類就變成指某幾顆元素，而那個字也掛在 header / 手機抽屜這些沒地址的地方，對位會擋。要指某幾顆就寫地址，要改字義就只寫 class。
- ⚠️ 同一顆 block 裡若有多顆同地址（例如重複的段落），結構偽類目前**選不到其中一顆** —— 對位會把偽類丟掉、整批都改到，並出 `target_retargeted` 警告。看到這個警告就停下回報，不要送。
...
**token 字典用 `design-get-dictionary` 拿**（見 [02-seat-setup](02-seat-setup.md) §3）；挑 block 用 `block-browse`（§0 那條）。
...
2. ⭐ **改了桌機字級，就一定要交代平板 / 手機** —— 2026-09-11 起**不寫會被擋下、送不出去**（站主定調）。
   ...
   - **② 交給機器**：畫稿**最前面加一行** `/* hz-ladder: follow */`（整份畫稿的字級改動都跟階梯），
     ...
     ⚠️ **② 階段前台的平板 / 手機仍然吃桌機字級，這是正常的** —— 階梯要到 ③ 收回才推，⛔ 別以為畫壞了去補 `@media`（要在 ② 就看到手機的樣子，改用 ① 自己寫 `@media`）。
...
> [01-seat-rules](01-seat-rules.md)「什麼不准做」第 7 條只放一句指過來，**規矩的主人在這一節**。
...
- **外部座位（手上沒有本機 repo / 容器的）一律用瀏覽器量**（Chrome DevTools `evaluate_script` 跑 `getComputedStyle`），⛔ 不靠本機 `curl` 抓前台判斷長相；非得用 `curl -k`（跳過憑證檢查）是繞路，回報時要明講。lint / locate 只證明「規則對得上」，不回量到的值。
...
> 規矩（誰決定、同站一次一頁、跑前告知站主）在 [01-seat-rules](01-seat-rules.md)；工具行為（拒跑、partial、續跑、題型選項、申報碼）在 `playbook-get name="guide/design/writeback"`。
> **這一節只講怎麼問站主。**
...
>
> ⚠️ 定調輪的計畫書裡，按鈕 / 字距 / 行高 / 卡片這幾類的「建議」欄目前可能寫「只有這頁」—— 那是工具還沒跟上，定調輪一律照站主定的「全站」選，不要照建議欄。
...
- 字典：用 `design-get-dictionary` 拿（見 [02-seat-setup](02-seat-setup.md) §3），每次呼叫拿到的都是現況。
