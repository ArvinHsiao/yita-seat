...
> ⭐ **先讀 [seat-brief](seat-brief.md)**（座位總稿，站主 2026-09-07 定稿）:② 盡量發揮、③ 才收斂;
> 你唯一要守的是「寫在草稿層、值寫清楚」。**本頁任何一句與它衝突，以它為準**，並回報那一句。
>
> 你是「座位」。你先用既有 block 把一頁排出來（①），再把它畫成想要的樣子（②）。你畫的**每一筆都是 hz 標籤**：內容填進 block 的欄位，長相寫成 token 變數。畫面會即時跟著變；畫對了，長相那部分最後會寫回全站的 design token（③，**⛔ 不是你跑** —— 由 host 或指定操作手跑，見 [seat-setup](seat-setup.md) §6；題目由站主答）。**先照 §0 的四段走，每一段走完停下來給站主看；聽不準就先問。**
>
...
| **③ 收回** | **⛔ 不是你跑**（2026-09-09 起改由 host 或指定操作手跑，規矩的主人在 [seat-setup](seat-setup.md) §6）。你只要：草稿層**留著別清** → 回報「這頁可以收了」→ 題目端過來時用白話答 | 見 [seat-setup](seat-setup.md) §6 | ⛔ 不要自己跑 `writeback.mjs` / `apply-plan.mjs`；⛔ 題目**不准自己選**（那是「改全站」還是「只改這頁」的差別，你看不到別頁） | 站主逐題回答 |
...
- **① 排頁之前先讀一次 block 字典**（`block-get-dictionary`，不帶參數一次讀完，見 [seat-setup](seat-setup.md) §3）：這個站裝了哪些 block、各適合放哪 / 不適合、有幾段文字幾個圖欄、能不能重複、面的模式。一次讀完比逐類翻 `block-list` 快。⚠️ **它是機器生成的第二視圖，不完整以 `block-list` 為準**（沒有 role、按鈕數、圖張數 —— 目錄本身就沒這幾欄；要看某顆的實際槽位去 `block-get-template`）。每次呼叫拿到的都是現況，懷疑舊了就重新呼叫一次。
...
- 兩種模式下**你的畫法完全一樣**（一樣寫草稿層、一樣照 §A-3 從字典選值）。差別在 ③ 收回時怎麼歸帳 —— 那一步**不是你跑**（見 [seat-setup](seat-setup.md) §6），但每一題都要問過站主。
...
- **翻不動的它會指名**（第幾行、你寫的原文、為什麼），改那一條再跑一次。**有一條對不到就整份不送。**
  它會說哪 18 種話、每一種你該怎麼改 → [seat-setup](seat-setup.md) §5-3（那張表是唯一一份，這裡不重複）；另有 3 個**不擋、照樣送**的警示碼也在那一節。
...
**兩本字典都用 MCP 工具拿**（見 [seat-setup](seat-setup.md) §3）：`design-get-dictionary`（值，這一節講的）與 `block-get-dictionary`（block，§0 那條）。
...
- **不能**：在按鈕上直接換字型（字型跟著文字的 `typo.NN` 走，見 B-2）；按鈕文字字級也走 `button.NN` 的 typo（`typo.09/10`），不直接寫在按鈕上
  —— ⚠️ 真寫了會落殘留 `slot_has_no_facet`（**按鈕／徽章的槽收不下「字」**，2026-09-09 起有專屬的碼；意思與該怎麼辦見 [seat-setup](seat-setup.md) §5-4）
...
> [seat-brief](seat-brief.md) 邊界第 11 條只放一句指過來，**規矩的主人在這一節**。
...
- ~~面的底色在草稿層寫 hex ③ 帶不走~~ —— **2026-09-07（A91）修掉了**：`--hz-surface-NN-bg: #2A2018` 現在收得回，
  ...
  `add_not_authorized` 並同時出一題，不會靜默 —— 2026-09-09 起，見 [seat-setup](seat-setup.md) §6）。
...
- 字典：用 `design-get-dictionary` 拿（見 [seat-setup](seat-setup.md) §3），每次呼叫拿到的都是現況。
