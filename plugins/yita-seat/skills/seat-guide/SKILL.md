...
> ⭐ **先讀 [seat-brief](../seat-brief/SKILL.md)**（座位總稿，站主 2026-09-07 定稿）:② 盡量發揮、③ 才收斂;
> 你唯一要守的是「寫在草稿層、值寫清楚」。**本頁任何一句與它衝突，以它為準**，並回報那一句。
>
> 你是「座位」。你先用既有 block 把一頁排出來（①），再把它畫成想要的樣子（②）。你畫的**每一筆都是 hz 標籤**：內容填進 block 的欄位，長相寫成 token 變數。畫面會即時跟著變；畫對了，長相那部分最後會寫回全站的 design token（③，**⛔ 不是你跑** —— 由 host 或指定操作手跑，見 [seat-setup](../seat-setup/SKILL.md) §6；題目由站主答）。**先照 §0 的四段走，每一段走完停下來給站主看；聽不準就先問。**
>
...
|---|---|---|---|---|
| **0 骨架** | 聊需求時**先問一題**：「這頁沿用站上的字典，還是這頁全部新開？」（見 §A-1，沒說就是沿用）→ 再列「段落清單」：每段一句用途 + 候選 block 1–2 顆（附一句理由） | `block-list`（依 category 查） | 不插、不建頁 | 清單 + 這輪是**沿用**還是**新開**。站主點頭才開始 |
| **① 形狀** | 建頁、插 block、調 repeater 格數、藏多餘格、版面屬性、錨點；**站主已經給的文字**一趟 batch 填進去 | `page-batch`（add-section / fill-page-text / set-item-visibility / set-block-anchor）、`block-set-attribute` | **不寫新文案、不打磨字**；沒給文字的槽留示範文字（示範文字在這一段**不是問題**）；不寫草稿層、不碰背景 | 前台網址 + 「哪幾顆請你看形狀」。站主逐顆說**留 / 換**；換的就移除重插，文字跟著搬 |
| **② 感覺** | 寫草稿層（色、字、留白、按鈕、動效、**漸層與純色底**）、背景**圖 / 影片**走 `block-set-surface`；每次寫完跑 lint、截圖 | §A–§C | 不改 HTML、不 `!important`、不碰 token | 網址 + 截圖 + lint summary。站主逐顆說**好 / 不好**。⚠️ **② 是畫畫，不是選版型**：座位**自己**不要為了畫出某個效果去翻別的 block 來套 —— 畫不出來就回報。但 ① 只是第一版，站主看了畫面**隨時會說換 / 調 block**，那是正常的：站主說換就換，換完那顆重畫它（站主 2026-09-09 定調）。⚠️ **動到 header / footer 的字要在回報裡單獨列一段「H/F 改了什麼」** —— 那是全站共用，不是這一頁的事（見 §B-1） |
...
| **③ 收回** | **⛔ 不是你跑**（2026-09-09 起改由 host 或指定操作手跑，規矩的主人在 [seat-setup](../seat-setup/SKILL.md) §6）。你只要：草稿層**留著別清** → 回報「這頁可以收了」→ 題目端過來時用白話答 | 見 [seat-setup](../seat-setup/SKILL.md) §6 | ⛔ 不要自己跑 `writeback.mjs` / `apply-plan.mjs`；⛔ 題目**不准自己選**（那是「改全站」還是「只改這頁」的差別，你看不到別頁） | 站主逐題回答 |

- ⚠️ **讀這份手冊與字典用 `Read` 工具，⛔ 不要 `cat`** —— 手冊 40KB 出頭，超過 Bash 輸出上限會被截斷，你會以為讀完了其實少一截，然後分好幾趟重讀（實測有人為此多花 4 趟）。`Read` 一次整份進得來。
- **文字的規則一句話**：有給的就填，沒給的不編。填字很便宜（一趟 batch），貴的是寫文案和改文案 —— 所以形狀沒定之前不寫新字。
- **① 排頁之前先讀一次 block 字典**（`block-get-dictionary`，不帶參數一次讀完，見 [seat-setup](../seat-setup/SKILL.md) §3）：這個站裝了哪些 block、各適合放哪 / 不適合、有幾段文字幾個圖欄、能不能重複、面的模式。一次讀完比逐類翻 `block-list` 快。⚠️ **它是機器生成的第二視圖，不完整以 `block-list` 為準**（沒有 role、按鈕數、圖張數 —— 目錄本身就沒這幾欄；要看某顆的實際槽位去 `block-get-template`）。每次呼叫拿到的都是現況，懷疑舊了就重新呼叫一次。
...
- **順手記帳**：從開工到站主第一次看到畫面，你呼叫了幾趟；哪幾趟是補呼叫（工具回的東西不夠、得再叫一次）。這是 host 要修流程的依據。
...
- 兩種模式下**你的畫法完全一樣**（一樣寫草稿層、一樣照 §A-3 從字典選值）。差別在 ③ 收回時怎麼歸帳 —— 那一步**不是你跑**（見 [seat-setup](../seat-setup/SKILL.md) §6），但每一題都要問過站主。
...
送出時跑一次**對位**（seat-setup §5-2），它會把你的選擇器翻成系統認得的形狀，
...
- **翻不動的它會指名**（第幾行、你寫的原文、為什麼），改那一條再跑一次。**有一條對不到就整份不送。**
  它會說哪 18 種話、每一種你該怎麼改 → [seat-setup](../seat-setup/SKILL.md) §5-3（那張表是唯一一份，這裡不重複）；另有 3 個**不擋、照樣送**的警示碼也在那一節。
...
**兩本字典都用 MCP 工具拿**（見 [seat-setup](../seat-setup/SKILL.md) §3）：`design-get-dictionary`（值，這一節講的）與 `block-get-dictionary`（block，§0 那條）。
...
- **header / footer 一起換色是預期的，但它是「全站共用」**：H/F 用的是專屬色格 `color.22–36`（字典 label 會標），在 `:root` 改它們就讓頁首頁尾與這頁同調。站主 2026-09-06 定調這不算越界。
  ⚠️ **但只有站主明說「H/F 也要改」時才動它**。你在這一頁改 H/F，只有這一頁看得到；③ 寫回的卻是站級 token，**全站每一頁的頁首頁尾都會跟著變**。所以 ③ 預設**不寫** H/F（要寫得加 `--hf=include`），而你若動了，**回報時要單獨列一段「H/F 改了什麼」**，讓站主決定要不要。
...
- ⚠️ **字級只能靠「換一個字」表達**，而換字的條件是「字級對得上 **且其餘欄位（字型／字重／行高／字距）與這句現在綁的字一樣**」。
  ...
  要嘛接受開一格新的字（③ 跑的時候加 `--auto-add`，或站主在問卷上答「開新格」），要嘛改用「改字義」（`.hz-typo-typo-NN { --_hz-fs: … }`）。
...
- **不能**：在按鈕上直接換字型（字型跟著文字的 `typo.NN` 走，見 B-2）；按鈕文字字級也走 `button.NN` 的 typo（`typo.09/10`），不直接寫在按鈕上
  —— ⚠️ 真寫了會落殘留 `slot_has_no_facet`（**按鈕／徽章的槽收不下「字」**，2026-09-09 起有專屬的碼；意思與該怎麼辦見 [seat-setup](../seat-setup/SKILL.md) §5-4）
...
> [seat-brief](../seat-brief/SKILL.md) 邊界第 11 條只放一句指過來，**規矩的主人在這一節**。
...
1. 樣式草稿層用 **`design-update-style-layer`** 寫進站台（2026-09-06 起，不再進容器下 WP-CLI）：
   ```
   design-update-style-layer { page_id: <PAGE_ID>, css: "<你寫的整段草稿層 CSS>" }
   ```
   - 送 `css: ""` 就是**清空**這一頁的草稿層。
   - 只放 token 變數與長相；⛔ 背景**圖 / 影片**是 block 資料，走 `block-set-surface`。**漸層與純色照畫**（見 §B-1）。
   - 上限約 65,536 字元。消毒在 plugin 端做，你不用自己先清。
...
4. **用眼睛看**：Chrome DevTools 截圖（`navigate_page` 換 `?x=` → `take_screenshot` fullPage jpeg → `Read` 看圖），不滿意再改。
...
0b. **③ 問了幾題**（跑完 ③ 才知道；由 host 補上）—— 題數多代表這一輪用了很多「新的值」，
...
## E. 實測踩過的坑（2026-09-06 夜三位座位五輪，每條都有頁面證據；讀完再開工）

- **`writeback.mjs plan` 在新開模式回 exit 1 不是失敗**：那是 `summary.unassigned`（`design_set` 待 apply-plan 回填）。exit 3 才是「有題沒答」。
- **`apply-plan.mjs` 尾巴「記得清草稿層」是固定提示**，計畫書最後一筆本來就是清草稿（`design-update-style-layer css:""`），跑完前台 `hz-style-layer:start` = 0 就是清了，不用再做一次。
- ~~面的底色在草稿層寫 hex ③ 帶不走~~ —— **2026-09-07（A91）修掉了**：`--hz-surface-NN-bg: #2A2018` 現在收得回，
  ③ 會開一格新色票再綁上去（要授權：跑 `--auto-add`，**或站主在問卷上答「開新格」**；沒授權會落殘留
  `add_not_authorized` 並同時出一題，不會靜默 —— 2026-09-09 起，見 [seat-setup](../seat-setup/SKILL.md) §6）。
  **直接寫 hex 就好**，不必先自己在 `:root` 開色。
- **草稿層寫字距用 em、行高用小數都可以，但寫回時會被對到檔位**：行高只有 1.1 / 1.2 / 1.3 / 1.4 / 1.5 / 1.6 / 1.8，字距只有九檔（正常 0em、寬 0.025em、更寬 0.05em…）。計畫書「吸附」段會列出「你寫的 → 對到的」，看一眼再答題。
- **③ 的影響面數字（幾頁幾顆）在驗收頁很多的站會很大**（三百多頁驗收頁），客戶站不會；讀到「⚠️ N 顆 block 的模板預設讀不到（影響範圍可能低估）」時，那個數字是低估。

## 附錄

- 字典：用 `design-get-dictionary` 拿（見 [seat-setup](../seat-setup/SKILL.md) §3），每次呼叫拿到的都是現況。
