...
> ⭐ **先讀 [01-seat-rules](01-seat-rules.md)**。本頁任何一句與它衝突，**以它為準**，並回報那一句。
> **這頁只講「怎麼開始」** —— 接線、開工前拿什麼、開座位、卡住看哪。
> **畫稿怎麼寫**看 [03-page-draw-guide](03-page-draw-guide.md)；**工具怎麼叫、參數、失敗碼**一律 `playbook-get`（不知道從哪查就先 `name="lite-index"`）。
...
> 後面的頁大多會直接吸附到剛定好的預設、題目自動變少。定調輪怎麼答見 [03-page-draw-guide](03-page-draw-guide.md) §C-3。
...
**③ 由座位跑、站主答題**（[01-seat-rules](01-seat-rules.md)）。兩支工具：`design-writeback-plan`（出題，反覆帶答案重產到可以執行）→ `design-writeback-apply`（寫回、清草稿層、驗收）。
...
4. **怎麼問站主**看 [03-page-draw-guide](03-page-draw-guide.md) §C-3；拒跑、partial、續跑這些工具行為看 `playbook-get name="guide/design/writeback"`。
...
| 座位說「這個做不到」 | 表上沒有的東西要**記下來回報**，不是硬做（[03-page-draw-guide](03-page-draw-guide.md) §A-6） |

---

## 8. 相關文件

- 牢記的那一頁：[01-seat-rules](01-seat-rules.md)
- 畫稿怎麼寫、③ 怎麼問：[03-page-draw-guide](03-page-draw-guide.md)
